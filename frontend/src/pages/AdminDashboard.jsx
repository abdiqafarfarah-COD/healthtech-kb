    ...
    <div className="page-container admin-dashboard">
      ...
      <div className="admin-stat-grid">
        <div className="admin-stat-box">
          <div className="admin-stat-value">{stats.total_articles}</div>
          <div className="admin-stat-label">Total Articles</div>
        </div>
        <div className="admin-stat-box">
          <div className="admin-stat-value">{stats.published_articles}</div>
          <div className="admin-stat-label">Published</div>
        </div>
        <div className="admin-stat-box">
          <div className="admin-stat-value">{stats.draft_articles}</div>
        ...
        <div className="admin-stat-box">
          <div className="admin-stat-value">{stats.total_categories}</div>
          <div className="admin-stat-label">Categories</div>
        </div>
      </div>

      <div className="admin-section-label">Chatbot Activity</div>
      <div className="admin-stat-grid admin-stat-grid-3">
        <div className="admin-stat-box">
          <div className="admin-stat-value">{stats.total_chats}</div>
          <div className="admin-stat-label">Total Conversations</div>
        </div>
        <div className="admin-stat-box">
          <div className="admin-stat-value">{stats.unanswered_chats}</div>
          <div className="admin-stat-label">Unanswered Questions</div>
        </div>
        <div className="admin-stat-box">
          ...
          <div className="admin-stat-label">Answer Rate</div>
        </div>
      </div>

      <div className="admin-section-label">Users by Role</div>
      <div className="admin-role-strip">
        {Object.entries(stats.users_by_role).map(([role, count]) => (
          <div key={role} className="admin-role-box">
          ...
          {stats.recent_searches.length === 0 ? (
            ...
            <ul className="admin-panel-list">
              {stats.recent_searches.map((s, i) => (
                <li key={i}>
                  <span>"{s.query}"</span>
                  <span className="admin-panel-metric">
